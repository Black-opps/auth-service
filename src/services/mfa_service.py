...
from ..models.mfa import MFAChallenge, MFADevice
...
class MFAService:
    ...
    async def setup_totp(
        self, user_id: UUID, device_name: str = "Default"
        ...
        provisioning_uri = pyotp.totp.TOTP(secret).provisioning_uri(
            name=user.email, issuer_name=self.issuer_name
        )
    ...
    async def verify_challenge(self, challenge_id: UUID, code: str) -> bool:
        ...
        challenge = (
            self.db.query(MFAChallenge).filter(MFAChallenge.id == challenge_id).first()
        )
    ...
    async def disable_mfa(self, user_id: UUID, password: str) -> bool:
        ...
        # Deactivate devices
        self.db.query(MFADevice).filter(MFADevice.user_id == user_id).update(
            {"is_active": False}
        )
