...
class AuthService:
    ...
    async def authenticate(
        ...
        if user.mfa_enabled:
            ...
            raise MFARequiredError(
                mfa_challenge_id=str(challenge.id), mfa_method="totp"
            )
    ...
    async def create_tokens(
        self, user: User, session: UserSession = None
    ) -> Dict[str, Any]:
        """
    ...
    async def refresh_access_token(self, refresh_token: str) -> Dict[str, Any]:
        ...
        if db_refresh.session_id:
            session = (
                ...
                .filter(UserSession.id == db_refresh.session_id)
                .first()
            )
        else:
            session = None
    ...
    async def logout(
        self, user_id: UUID, refresh_token: str = None, logout_all: bool = False
    ):
        """
