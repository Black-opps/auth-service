...
class KeyService:
    ...
    async def verify_key(
        self, raw_key: str, ip_address: str = None
    ) -> Optional[APIKey]:
        """
        Verify an API key.
        ...
        if not api_key:
            logger.warning(
                f"API key not found or inactive: {mask_sensitive_data(raw_key)}"
            )
            return None
        ...
        if ip_address and api_key.ip_restrictions:
            if ip_address not in api_key.ip_restrictions:
                logger.warning(
                    f"API key IP restriction failed: {ip_address} not in {api_key.ip_restrictions}"
                )
                return None
    ...
    async def rotate_key(
        self, key_id: UUID, tenant_id: UUID, expires_in_days: Optional[int] = 365
    ) -> tuple[APIKey, str]:
        """
